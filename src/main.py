    ...
    def get(self):
        ...
        for search_keyword in search_keywords:
            try:
                ...
                taskqueue.add(url='/feed/keyword', method='GET', params={'keyword_id' : search_keyword.key().id()})
...
class UpdateCacheIdsCron(webapp.RequestHandler):
    def get(self):
        search_cache_query = SearchCache().all()
        search_cache_query.order('-tweeted_at')
...
class TweetFeed(webapp.RequestHandler):
    def get(self):
...
class KeywordFeed(webapp.RequestHandler):
    def get(self):
        keyword_id = self.request.get('keyword_id')
        ...
        if len(cache_ids) > 0:
            ...
            if data is None:
                ...
                if search_cache is not None:
                    ...
                    created_at = search_cache.tweeted_at + datetime.timedelta(hours=9)
                    ...
                    if len(search_cache.text) < 60:
                        data = {'id': search_cache.key().id(),
                                ...
                                'delta':delta_str,
                                'created_at': created_at.strftime('%Y-%m-%d %H:%M:%S (JST)'),
                                'time_zone': time_zone}
...
application = webapp.WSGIApplication(
                                     [('/', MainPage),
                                      ...
                                      ('/feed/keyword', KeywordFeed),
                                      ('/feed/tweet', TweetFeed),
                                      ('/cron/add', AddTaskCron),
                                      ('/cron/delete', DeleteOldCacheCron),
                                      ('/cron/update', UpdateCacheIdsCron),
                                      ('/error', ErrorPage)],
                                     debug=False)
